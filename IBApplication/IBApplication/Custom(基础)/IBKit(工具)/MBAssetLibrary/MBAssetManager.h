...
/// Asset 授权的状态
typedef NS_ENUM(NSUInteger, MBAssetAuthorizationStatus) {
    MBAssetAuthorizationStatusNotDetermined,      // 还不确定有没有授权
    MBAssetAuthorizationStatusAuthorized,         // 已经授权
    MBAssetAuthorizationStatusNotAuthorized       // 手动禁止了授权
};

NS_ASSUME_NONNULL_BEGIN

typedef void (^MBWriteAssetCompletionBlock)(MBAsset * _Nullable asset, NSError *error);

/// 保存图片到指定相册（传入 UIImage）
extern void MBImageWriteToSavedPhotosAlbumWithAlbumAssetsGroup(UIImage *image, MBAssetGroup *albumAssetsGroup, MBWriteAssetCompletionBlock completionBlock);

/// 保存图片到指定相册（传入图片路径）
extern void MBSaveImageAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *imagePath, MBAssetGroup *albumAssetsGroup, MBWriteAssetCompletionBlock completionBlock);

/// 保存视频到指定相册
extern void MBSaveVideoAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *videoPath, MBAssetGroup *albumAssetsGroup, MBWriteAssetCompletionBlock completionBlock);

/**
 *  构建 MBAssetManager 这个对象并提供单例的调用方式主要出于下面两点考虑：
 *  1. 保存照片/视频的方法较为复杂，为了方便封装系统接口，同时灵活地扩展功能，需要有一个独立对象去管理这些方法。
 *  2. 使用 PhotoKit 获取图片，基本都需要一个 PHCachingImageManager 的实例，为了减少消耗，
 *     MBAssetManager 单例内部也构建了一个 PHCachingImageManager，并且暴露给外面，方便获取
 *     PHCachingImageManager 的实例。
 */
@interface MBAssetManager : NSObject
 ...
 *
 *  @warning 无论用户保存到哪个自行创建的相册，系统都会在“相机胶卷”相册中同时保存这个图片。
 *           原因请参考 MBAssetManager 对象的保存图片和视频方法的注释。
 *  @warning 无法通过该方法把图片保存到“智能相册”，“智能相册”只能由系统控制资源的增删。
 */
