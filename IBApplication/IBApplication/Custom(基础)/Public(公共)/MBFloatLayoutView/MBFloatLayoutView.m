...
#import "MBFloatLayoutView.h"
#import "UIMacros.h"

#define ValueSwitchAlignLeftOrRight(valueLeft, valueRight) ([self shouldAlignRight] ? valueRight : valueLeft)

const CGSize MBUIFloatLayoutViewAutomaticalMaximumItemSize = {-1, -1};

@implementation MBFloatLayoutView
...
- (void)didInitialize {
    ...
    self.maximumItemSize = MBUIFloatLayoutViewAutomaticalMaximumItemSize;
}
...
- (CGSize)layoutSubviewsWithSize:(CGSize)size shouldLayout:(BOOL)shouldLayout {
    ...
    CGSize maximumItemSize = CGSizeEqualToSize(self.maximumItemSize, MBUIFloatLayoutViewAutomaticalMaximumItemSize) ? CGSizeMake(size.width - UIEdgeInsetsGetHorizontalValue(self.padding), size.height - UIEdgeInsetsGetVerticalValue(self.padding)) : self.maximumItemSize;
    NSInteger line = -1;
