...
#import "MBAutoScrollLabel.h"
#import <QuartzCore/QuartzCore.h>

// Notification strings
NSString *const kMBAutoScrollLabelControllerRestartNotification = @"MBAutoScrollLabelViewControllerRestart";
NSString *const kMBAutoScrollLabelShouldLabelizeNotification = @"MBAutoScrollLabelShouldLabelizeNotification";
NSString *const kMBAutoScrollLabelShouldAnimateNotification = @"MBAutoScrollLabelShouldAnimateNotification";
NSString *const kMBAutoScrollLabelAnimationCompletionBlock = @"MBAutoScrollLabelAnimationCompletionBlock";
...
@implementation MBAutoScrollLabel

#pragma mark - Class Methods and handlers

+ (void)restartLabelsOfController:(UIViewController *)controller {
    [MBAutoScrollLabel notifyController:controller
                            withMessage:kMBAutoScrollLabelControllerRestartNotification];
}

+ (void)controllerViewWillAppear:(UIViewController *)controller {
    [MBAutoScrollLabel restartLabelsOfController:controller];
}
...
+ (void)controllerLabelsShouldLabelize:(UIViewController *)controller {
    [MBAutoScrollLabel notifyController:controller
                            withMessage:kMBAutoScrollLabelShouldLabelizeNotification];
}

+ (void)controllerLabelsShouldAnimate:(UIViewController *)controller {
    [MBAutoScrollLabel notifyController:controller
                            withMessage:kMBAutoScrollLabelShouldAnimateNotification];
}
...
- (void)setupLabel {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewControllerShouldRestart:) name:kMBAutoScrollLabelControllerRestartNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(labelsShouldLabelize:) name:kMBAutoScrollLabelShouldLabelizeNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(labelsShouldAnimate:) name:kMBAutoScrollLabelShouldAnimateNotification object:nil];
...
#pragma mark - MBAutoScrollLabel Heavy Lifting
...
- (void)scrollAwayWithInterval:(NSTimeInterval)interval delayAmount:(NSTimeInterval)delayAmount shouldReturn:(BOOL)shouldReturn {
    ...
    [awayAnim setValue:@(YES) forKey:kMBAutoScrollLabelAnimationCompletionBlock];
...
- (void)scrollContinuousWithInterval:(NSTimeInterval)interval
    ...
    [labelAnimation setValue:@(YES) forKey:kMBAutoScrollLabelAnimationCompletionBlock];
...
- (void)setNumberOfLines:(NSInteger)numberOfLines {
    // By the nature of MBAutoScrollLabel, this is 1
    [super setNumberOfLines:1];
}

- (void)setAdjustsFontSizeToFitWidth:(BOOL)adjustsFontSizeToFitWidth {
    // By the nature of MBAutoScrollLabel, this is NO
...
- (void)setMarqueeType:(MBMarqueeType)marqueeType {
...
@implementation UIView (MBAutoScrollLabelHelpers)
// Thanks to Phil M
...
@implementation CAMediaTimingFunction (MBAutoScrollLabelHelpers)
...
- (CGFloat)solveTForY:(CGFloat)y_0 withEpsilon:(CGFloat)epsilon controlPoints:(NSArray *)controlPoints
{
    ...
    for (int i = 0; i < 15; i++) {
        ...
        if (fabs(df0) < 1e-6) {
            NSLog(@"MBAutoScrollLabel: Newton's Method failure, small/zero derivative!");
            break;
        }
        // Else recalculate t1
        t1 = t0 - f0/df0;
    ...
    NSLog(@"MBAutoScrollLabel: Failed to find t for Y input!");
    return t0;
}
