...
@class MBPopupController;

typedef NS_ENUM(NSUInteger, MBPopupControllerAnimation) {
    MBPopupControllerAnimationFade,    // 渐现渐隐，默认
    MBPopupControllerAnimationPopup,   // 从中心点弹出
    MBPopupControllerAnimationSlide    // 从下往上升起
};

@protocol MBPopupControllerProtocol <NSObject>

@optional

/**
 *  当浮层以 UIViewController 的形式展示（而非 UIView），并且使用 modalController 提供的默认布局时，则可通过这个方法告诉 popupController 当前浮层期望的大小。如果 modalController 实现了自己的 layoutBlock，则可不实现这个方法，实现了也不一定按照这个方法的返回值来布局，完全取决于 layoutBlock。
 *  @param  controller  当前的popupController
 *  @param  keyboardHeight 当前的键盘高度，如果键盘降下，则为0
 *  @param  limitSize   浮层最大的宽高，由当前 popupController 的大小及 `contentViewMargins`、`maximumContentViewWidth` 和键盘高度决定
 *  @return 返回浮层在 `limitSize` 限定内的大小，如果业务自身不需要限制宽度/高度，则为 width/height 返回 `CGFLOAT_MAX` 即可
 */
- (CGSize)preferredContentSizeInPopupController:(MBPopupController *)controller keyboardHeight:(CGFloat)keyboardHeight limitSize:(CGSize)limitSize;

@end

@protocol MBPopupControllerDelegate <NSObject>
 ...
 */
- (BOOL)shouldHidePopupController:(MBPopupController *)controller;

/**
 *  modalController 即将隐藏时的回调方法，在调用完这个方法后才开始做一些隐藏前的准备工作，例如恢复 window 的 dimmed 状态等。
 *  @param  controller  当前的popupController
 */
- (void)willHidePopupController:(MBPopupController *)controller;
 ...
 *  @param  controller  当前的popupController
 */
- (void)didHidePopupController:(MBPopupController *)controller;
 ...
 *  支持 3 种方式显示浮层：
 *
 *  1. **推荐** 新起一个 `UIWindow` 盖在当前界面上，将 `MBPopupController` 以 `rootViewController` 的形式显示出来，可通过 `supportedOrientationMask` 支持横竖屏，不支持在浮层不消失的情况下做界面切换（因为 window 会把背后的 controller 盖住，看不到界面切换）。
 *  可通过 shownInWindowMode 属性来判断是否在用这种方式显示。
 *  @code
 *  [popupController showWithAnimated:YES completion:nil];
 *  @endcode
 *
 ...
 *  @endcode
 *
 *  3. 将浮层作为一个 subview 添加到 `superview` 上，从而能够实现在浮层不消失的情况下进行界面切换，但需要 `superview` 自行管理浮层的大小和横竖屏旋转，而且 `MBPopupController` 不能用局部变量来保存，会在显示后被释放，需要自行 retain。横竖屏跟随当前界面的设置。
 *  可通过 shownInSubviewMode 属性来判断是否在用这种方式显示。
 *  @code
 *  self.popupController.view.frame = CGRectMake(50, 50, 100, 100);
 *  [self.view addSubview:self.popupController.view];
 *  @endcode
 *
 *  默认的布局会将浮层居中显示，浮层的大小可通过接口控制：
 *  1. 如果是用 `contentViewController`，则可通过 `preferredContentSizeInPopupController:keyboardHeight:limitSize:` 来设置
 *  2. 如果使用 `contentView`，或者使用 `contentViewController` 但没实现 `preferredContentSizeInPopupController:keyboardHeight:limitSize:`，则调用`contentView`的`sizeThatFits:`方法获取大小。
 ...
 *
 *  @warning 如果使用者retain了popupController，注意应该在`hideWithAnimated:completion:`里release
 *
 *  @see MBAlertController
 */
@interface MBPopupController : UIViewController

@property (nullable, nonatomic, weak) IBOutlet id<MBPopupControllerDelegate> delegate;
 ...
 */
@property (nullable, nonatomic, strong) IBOutlet UIViewController<MBPopupControllerProtocol> *contentViewController;
 ...
 *  @warning 这里的`completion`只会在你显式调用`hideWithAnimated:completion:`方法来隐藏浮层时会被调用，如果你通过点击`dimmingView`来触发`hideWithAnimated:completion:`，则completion是不会被调用的，那种情况下如果你要在浮层隐藏后做一些事情，请使用`delegate`提供的`didHidePopupController:`方法。
 */
 ...
 *  @param animated     是否以动画的形式隐藏
 *  @param completion   隐藏动画结束后的回调
 *  @warning 这里的`completion`只会在你显式调用`hideInView:animated:completion:`方法来隐藏浮层时会被调用，如果你通过点击`dimmingView`来触发`hideInView:animated:completion:`，则completion是不会被调用的，那种情况下如果你要在浮层隐藏后做一些事情，请使用`delegate`提供的`didHidePopupController:`方法。
 */
...
/**
 *  如果你有一个控件，内部通过 MBPopupController 实现显隐功能，那么这个控件建议实现这个协议，这样当 + [MBPopupController hideAllVisiblePopupControllerIfCan] 被调用的时候，可以通过 hidePopupControllerComponent 来隐藏你的控件，否则会直接调用 MBPopupController 的 hide 方法，那样可能导致你的控件无法正确被隐藏。
 */
@protocol MBPopupControllerComponentProtocol <NSObject>

@required
- (void)hidePopupControllerComponent;

@end


@interface MBPopupController (Manager)

/**
 *  判断当前App里是否有popupController正在显示（存在popupController但不可见的时候，也视为不存在）
 ...
 *  @see    shouldHidePopupController:
 *  @see    MBPopupControllerComponentProtocol
 *  @warning 当要隐藏一个 popupController 时，如果这个 modal 有实现 MBPopupControllerComponentProtocol 协议，则会调用它的 hidePopupControllerComponent 方法来隐藏，否则直接用 MBPopupController 的 hideWithAnimated:completion:
 */
